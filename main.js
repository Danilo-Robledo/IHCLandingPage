...
form.addEventListener("submit", (event) => {
  ...
  const fnumDoc = form.elements["numDoc"].value;
  const ftelefono = form.elements["telefono"].value;
  if (!fnombres || !fapellidos || !fnumDoc || !femail || !ftelefono) {
...
document.getElementById("notaForm").addEventListener("submit", function (e) {
  ...
  // Agregar alumno
  const nuevo = { nombres, apellidos, sede, nota };
...
function mostrarRanking() {
  ...
  estudiantes.forEach((est) => {
    ...
    texto.textContent = `${est.nombres} ${est.apellidos} (${est.sede}) – Nota: ${est.nota}`;
